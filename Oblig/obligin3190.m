...
function oppgave_5(h1, h2)
    ...
    subplot(1,2,1);
    x = filtered_seismogram1(:,50);
    plot(1:length(x), abs(x));
    xlim([230,300]);
    title("Chosen trase 50");
    xlabel("Normalized time (samples)");
    ylabel("Amplitude");
    
    subplot(1,2,2);
    x = filtered_seismogram1(:,150);
    plot(1:length(x), abs(x));
    xlim([340,420]);
    title("Chosen trase 150");
    xlabel("Normalized time (samples)");
    ylabel("Amplitude");
    s = offset1(150) - offset1(50);
    t = 356/1000 - 252/1000;
    v=s/t
    t = (s/1500)*1000
end
...
function oppgave_6(h1, h2)
    ...
    seisnmo = nmocorrection2(t,dt,offset1,filtered_seismogram1,vnmo);
end
